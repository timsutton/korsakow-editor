...
public class VideoTDG
{
	...
	public static int insert(long id, long version, String name, String filename, String subtitles) throws javax.xml.xpath.XPathExpressionException{
		Element element = DataRegistry.getHelper().appendElement("/korsakow/videos", NODE_NAME);
	...
	public static int delete(long id, long version) throws XPathExpressionException {
		NodeList list = DataRegistry.getHelper().removeNodes(XPathHelper.formatQuery("/korsakow/videos/Video[id=? and version=?]", id, version));
...
//		Element element = DataRegistry.getHelper().xpathAsElement(XPathHelper.formatQuery("/korsakow/videos/Video[id=? and version=?]", id, version));
		if (element == null)
			return 0;
		DataRegistry.getHelper().setLong(element, "id", id);
	...
	public static void  createVideoTable() throws XPathExpressionException {
		if (DataRegistry.getHelper().xpathAsElement("/korsakow/videos") == null) {
			DataRegistry.getHelper().appendElement("/korsakow", "videos");
		}
	}
	public static void dropTable () throws XPathExpressionException{
		DataRegistry.getHelper().removeNodes(XPathHelper.formatQuery("/korsakow/videos"));
	}
}
