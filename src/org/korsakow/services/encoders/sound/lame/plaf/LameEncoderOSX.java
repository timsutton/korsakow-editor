...
public class LameEncoderOSX extends LameEncoder
{
	public static class LameEncoderOSXDescription extends LameEncoder.LameEncoderDescription
	{
		public Class<? extends SoundEncoder> getEncoderClass() {
			return LameEncoderOSX.class;
		}
	}
	...
	protected Process createProcess(SoundFormat destFormat, File src, File dst) throws IOException
	{
		...
		cmds.add(ExternalsResourceManager.getExternalFile(ExternalsResourceManager.LAME_OSX).getAbsolutePath());
		cmds.add("--nohist");
		cmds.add("--disptime");
		cmds.add("60");
		cmds.add("--brief");
