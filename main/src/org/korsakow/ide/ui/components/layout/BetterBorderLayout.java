...
/*
 * COPY AND PASTED java.awt.BorderLayout
 * This Layout respects Maximum and Minimum Size
 */
...
public class BetterBorderLayout implements LayoutManager2,
	...
	if ((c=getChild(NORTH,ltr)) != null) {
	    ...
	    Dimension d = c.getPreferredSize();
	    int height = d.height;
	    height = Math.min(d.height, c.getMaximumSize().height);
	    height = Math.max(d.height, c.getMinimumSize().height);
	...
	if ((c=getChild(EAST,ltr)) != null) {
	    ...
	    Dimension d = c.getPreferredSize();
	    int width = d.width;
	    width = Math.min(d.width, c.getMaximumSize().width);
	    width = Math.max(d.width, c.getMinimumSize().width);
	...
	if ((c=getChild(CENTER,ltr)) != null) {
	    ...
	    height = Math.min(height, c.getMaximumSize().height);
