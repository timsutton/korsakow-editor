...
public class VerticalFlowLayout implements LayoutManager, java.io.Serializable {
		...
		synchronized (target.getTreeLock()) {
			...
			for (int i = 0; i < nmembers; i++) {
				...
				if (m.isVisible()) {
					...
					dim.width = Math.min(dim.width, max.width);
					if (firstVisibleComponent) {
						firstVisibleComponent = false;
					} else {
						dim.height += vgap;
					}
					dim.height += Math.min(d.height, max.height);
				}
			}
	...
	private void moveComponents(Container target, int x, int y, int width,
			int height, int colStart, int colEnd, boolean ltr) {
		synchronized (target.getTreeLock()) {
			switch (newAlign) {
			case TOP:
				y += ltr ? 0 : height;
			...
			for (int i = colStart; i < colEnd; i++) {
				Component m = target.getComponent(i);
				if (m.isVisible()) {
					int offsetX = (width - m.getWidth()) / 2;
					offsetX *= m.getAlignmentX();
					if (ltr) {
						m.setLocation(x + offsetX, y);
					} else {
						m.setLocation(x + offsetX, target.getHeight() - y
								- m.getHeight());
					}
					y += m.getHeight() + vgap;
				}
		...
		synchronized (target.getTreeLock()) {
			...
			for (int i = 0; i < nmembers; i++) {
				...
				if (m.isVisible()) {
					...
					d.height = Math.min(d.height, dMax.height);
					...
					if ((y == 0) || ((y + d.height) <= maxheight)) {
						...
						moveComponents(target, insets.left + hgap, y, maxheight
								- x, colw, start, i, ltr);
